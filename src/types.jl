...
function Base.getproperty(ctx::SHA1_CTX, fieldname::Symbol)
	...
	elseif fieldname === :state
		getfield(ctx, :state)::Vector{UInt32}
	elseif fieldname === :bytecount
		getfield(ctx, :bytecount)::UInt64
	elseif fieldname === :buffer
		getfield(ctx, :buffer)::Vector{UInt8}
	elseif fieldname === :W
		getfield(ctx, :W)::Vector{UInt32}
	else
...
function Base.getproperty(ctx::SHA2_CTX, fieldname::Symbol)
	if false
	elseif fieldname === :state
		getfield(ctx, :state)::Union{Vector{UInt32}, Vector{UInt64}}
	elseif fieldname === :bytecount
		getfield(ctx, :bytecount)::Union{UInt64, UInt128}
	elseif fieldname === :buffer
		getfield(ctx, :buffer)::Vector{UInt8}
	else
		error("type ", typeof(ctx), " has no field ", fieldname)
...
function Base.getproperty(ctx::SHA3_CTX, fieldname::Symbol)
	if false
	elseif fieldname === :state
		getfield(ctx, :state)::Vector{UInt64}
	elseif fieldname === :bytecount
		getfield(ctx, :bytecount)::UInt128
	elseif fieldname === :buffer
		getfield(ctx, :buffer)::Vector{UInt8}
	elseif fieldname === :bc
		getfield(ctx, :bc)::Vector{UInt64}
	else
		error("type ", typeof(ctx), " has no field ", fieldname)
	end
end
...
blocklen(::Type{SHA3_224_CTX}) = UInt64(25 * 8 - 2digestlen(SHA3_224_CTX))
blocklen(::Type{SHA3_256_CTX}) = UInt64(25 * 8 - 2digestlen(SHA3_256_CTX))
blocklen(::Type{SHA3_384_CTX}) = UInt64(25 * 8 - 2digestlen(SHA3_384_CTX))
blocklen(::Type{SHA3_512_CTX}) = UInt64(25 * 8 - 2digestlen(SHA3_512_CTX))

# short_blocklen is the size of a block minus the width of bytecount
short_blocklen(::Type{T}) where T <: SHA_CTX = blocklen(T) - 2sizeof(state_type(T))
...
# SHA1 is special; it needs extra workspace
"""
	SHA1_CTX()
